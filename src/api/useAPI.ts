...
export const useAPI = <T>(
  ...
  const url = ref(keyMaps[key]({ ...(isRef(pathVariables) ? pathVariables.value : pathVariables) }));
  ...
  if (cache) {
    ...
    const { data, error, mutate, isValidating } = useSWRV<T>(
      ...
      () => {
        ...
        switch (method) {
          case "GET":
            return fetcher(url.value, params, isRef(options) ? options.value : options);
          case "POST":
            return fetcherPost(url.value, isRef(body) ? body.value : body, isRef(options) ? options.value : options);
          case "DELETE":
            return deleter(url.value, isRef(options) ? options.value : options);
          case "PUT":
            return updater(url.value, isRef(body) ? body.value : body, isRef(options) ? options.value : options);
          default:
            return fetcher(url.value, params, isRef(options) ? options.value : options);
        }
      },
    ...
    function getRequest(): any {
      switch (method) {
        case "POST":
          return getAxiosClient().post<T>(url.value, isRef(body) ? body.value : body, {
            params,
            ...(isRef(options) ? options.value : options),
          });
        case "DELETE":
          return getAxiosClient().delete<T>(url.value, { ...(isRef(options) ? options.value : options) });
        case "PUT":
          return getAxiosClient().put<T>(url.value, isRef(body) ? body.value : body, {
            params,
            ...(isRef(options) ? options.value : options),
          });
        default:
          return getAxiosClient().get<T>(url.value, { params, ...(isRef(options) ? options.value : options) });
      }
    }
