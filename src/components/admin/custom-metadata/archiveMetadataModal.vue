<template>
  ...
  >
    ...
    <div class="mb-5 font-size-h5">
      ...
      <span class="font-weight-bold">{{ businessMetadata.displayName }}</span>
      business metadata?
    </div>
...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    const handleArchive = () => {
      ...
      let businessMetadata = JSON.parse(JSON.stringify(props.businessMetadata));
      businessMetadata = {
        ...businessMetadata,
        isArchived: true,
      };
      ...
      watch(
        ...
        error => {
          // error
          deleteLoading.value = false;
          console.log("🚀 ~ file: index.vue ~ line 89 ~ handleArchive ~ error", error);
          if (error.response) {
