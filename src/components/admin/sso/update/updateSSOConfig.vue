<template>
            ...
            <template #label>
              <span class="flex flex-col h-48">
                ...
                <span class="leading-3"
                  >Paste or
                  <ImportText @importCertificate="importCertificate" />
                </span>
              </span>
            </template>
  ...
  <div v-else>
    <div class="w-full p-4 text-center h-96">
      <img :src="emptySSOImage" class="h-48 p-2 mx-auto" alt="Empty Image" />
      <strong class="block py-8 text-lg"
        >Oops! SSO provider doesn't exist.</strong
      >
      <router-link to="/admin/sso">
        <a-button style="width: 150px" type="primary"> Go Back </a-button>
      </router-link>
    </div>
  </div>
</template>
...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    onMounted(() => {
      const data: any = toRaw(ssoProvider.value);
      ssoForm.singleSignOnServiceUrl = data?.config?.singleSignOnServiceUrl;
