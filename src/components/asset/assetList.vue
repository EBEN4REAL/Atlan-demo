  ...
  <div :style="listStyle">
    ...
    >
      <div class="px-8 text-center">
        <div class="">
          <i class="fal fa-exclamation-triangle display-4 text-danger"></i>
        </div>
    ...
    >
      ...
      <div class="mt-3 text-center">
        <slot name="emptyCopy">
          ...
          <span class="text-muted">Try changing filters or search keyword</span>
        </slot>
      </div>
    </div>
    <simplebar v-else :style="{ 'max-height': maxHeight }" ref="assetsListRef">
      <AssetItemCard
      ...
      >
        ...
        >
          ...
          >
            ...
            ></i
            ><span class="text-muted font-size-sm">Unlinking...</span>
          </span>
          <span
            v-if="
          ...
          <span
            ...
            "
          >
            <i class="mr-1 far fa-check-circle text-success font-size-sm"></i
            ><span class="text-muted font-size-sm">Asset unlinked</span>
          </span>
        </div>
      </AssetItemCard>
      <div
      ...
      >
        <p class="mb-0 text-gray-500 font-w700">
          ...
          {{ limit ? limit - fetchedAssetCountInOneBatch : "" }} more assets...
        </p>
      </div>
      <div
        @click="loadMore"
...
export default defineComponent({
  ...
  props: {
    ...
    loadingStatus: {
      type: String,
      validator(value) {
        return ["loading", "error", "success", ""].indexOf(value) > -1;
      },
    },
    selectedAssetId: {
      type: String,
  ...
  setup(props, context) {
    ...
    const askAgainBeforeUnLinking = computed(
      () => store.state.glossary.askAgainBeforeUnLinking
    );
    ...
    const assetsListScroll = () => {
      ...
      scrollElement.onscroll = () => {
        if (
          !props.listComplete &&
          scrollElement.scrollHeight - scrollElement.offsetHeight - 1000 <=
    ...
    watch(loadingStatus, () => {
      if (loadingStatus === "success") {
        nextTick(() => {
          if (loadingStatus === "success" && assetsListRef.value) {
