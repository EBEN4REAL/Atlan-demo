...
export default defineComponent({
    ...
    setup(props, { emit }) {
        ...
        const getOwnerFilterAppliedString = (
            ...
            let str = ''
            if (usersLength)
                str += `${usersLength} ${usersLength > 1 ? 'users' : 'user'}`
        ...
        const getFilterValue = computed(() => {
            ...
            if (id === '__traitNames' && facetMap.value[id]) {
                ...
                const list = classificationList.value
                    .filter((i) =>
                        facetMap.value[id]?.classifications?.includes(i.name)
                    )
                    .map((i) => i.displayName)
            ...
            if (id === 'owners') {
                ...
                if (usersLength === 0 && groupsLength < 3 && groupsLength > 0) {
            ...
            Object.keys(facetMap.value[id]).forEach((key) => {
                if (facetMap.value[id][key].find((element) => element.value))
                    numOfAttributes += 1
            })
