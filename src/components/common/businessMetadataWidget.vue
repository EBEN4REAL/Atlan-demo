<template>
    <div class="border border-gray-200 rounded">
            ...
            <div class="flex justify-between">
                ...
                <a-popconfirm
                    ...
                    ><span
                        ><fa
                            icon="fal times"
                            class="cursor-pointer text-gray hover:text-red-600"
                        /> </span
                ></a-popconfirm>
            </div>
        </div>
        <span v-if="localState?.attributes?.length">
            <div
                v-for="(a, x) in localState.attributes"
                :key="x"
                class="px-2 mb-2"
            >
                <div class="flex justify-between">
                ...
                <span v-else class="">
                    ...
                    >
                        <a-date-picker
                            ...
                            @change="
                                (timestamp, string) =>
                                    handleDateChange(timestamp, string, x)
                            "
                        />
                    </span>
                    <div
                        v-else-if="
                            getDatatypeOfAttribute(a.typeName) === 'array<date>'
                        "
                        class=""
                    >
                        <div class="grid grid-cols-2 mb-2 gap-x-2 gap-y-1">
                            <div
                                v-for="(i, n) in multiInputs[a.name]"
                                :key="n"
                                class=""
                            >
                                <a-date-picker
                        ...
                        <div class="flex justify-end">
                            ...
                            >
                                <fa icon="fal plus" class="pushtop"></fa> add
                                more </a-tag
                            ><a-tag
                    ...
                    <div
                        v-else-if="
                            getDatatypeOfAttribute(a.typeName) === 'array<text>'
                        "
                        class="text-center"
                    >
                        <div class="grid grid-cols-2 mb-2 gap-x-2 gap-y-1">
                    ...
                    <div
                        v-else-if="
                            typeof getDatatypeOfAttribute(a.typeName) ===
                                'object' &&
                    ...
                    >
                        <a-select
                            ...
                            placeholder=""
                            :mode="
                                getDatatypeOfAttribute(a.typeName).isMultivalues
                                    ? 'tags'
                                    : ''
                            "
                            :options="
        ...
        <div v-else class="text-center">
            <p class="mt-2 text-xs text-gray">Please add an attribute</p>
        </div>
        ...
        setup(props, { emit }) {
            ...
            const confirmDeleteAttribute = (name) => {
                localState.value.attributes =
                    localState.value.attributes.filter((a) => a.name !== name)
                updateAttribute()
            }
            ...
            const selectAddAttribute = (a: object) => {
                if (a.typeName.includes('array'))
                    multiInputs.value[a.name] = ['', '']
            ...
            const updateAttribute = () => {
                emit(
                    'updateAttribute',
                    JSON.parse(JSON.stringify({ ...localState.value }))
                )
            }
            /**
            ...
            const handleEditMode = (name: string, type: string) => {
                const attribute = localState.value.attributes.find(
                    (a) => a.name === name
                )
                ...
                if (typeof type === 'object') console.log('enum', type)
                    ...
                    if (type.includes('date'))
                        multiInputs.value[name] = attribute.value.map(
                            (v: number) => v.toString()
                        )
            ...
            // TODO use composable businessMetadataHelper
            ...
            const availableAttributesToAdd = computed(() => {
                if (localState.value) {
                    const appliedAttributes = localState.value.attributes.map(
                        (a) => a.name
                    )
                    if (props.originalBM?.attributeDefs) {
                        return props.originalBM.attributeDefs.filter((a) => {
                            if (
                                ...
                                !JSON.parse(
                                    a.options.applicableEntityTypes
                                ).includes(props.assetType)
                            )
                                return false
                            return true
                        })
