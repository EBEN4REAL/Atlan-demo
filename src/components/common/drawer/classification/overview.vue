<template>
    <div class="px-4 py-5 overflow-hidden overflow-y-auto tab-content-wrapper">
        ...
        <div v-else class="h-full">
            <div class="mb-5 text-base font-bold text-gray-500">Overview</div>
            <div class="mb-6">
            ...
            <div class="mb-6">
                ...
                >
                    ...
                    <span class="text-gray-700">{{
                        lastUpdatedByUserName.includes(
            ...
            <div class="mb-6">
                ...
                >
                    ...
                    <span class="text-gray-700">{{
                        createdByUserName.includes('service-account-atlan-argo')
                            ? 'Atlan'
                            : createdBy.name
                    }}</span>
    ...
    export default defineComponent({
        ...
        setup(props, { emit }) {
            ...
            const lastUpdatedAt = computed(() =>
                dayjs().to(dayjs(classification.value.updateTime))
            )
            const createdAt = computed(() =>
                dayjs().to(dayjs(classification.value.createTime))
            )

            const createdByUserName = computed(
                () => classification.value.createdBy
            )
            const lastUpdatedByUserName = computed(
                () => classification.value.updatedBy
            )
            ...
            const createdBy = computed(() =>
                ...
                userList.value.filter(
                    ...
                    ? {
                          ...userList.value.filter(
                              (user) =>
                                  user.username === createdByUserName.value
                          )[0],
                      }
                    : {}
            )
            ...
            const lastUpdatedBy = computed(() =>
                ...
                userList.value.filter(
                    ...
                    ? {
                          ...userList.value.filter(
                              (user) =>
                                  user.username === lastUpdatedByUserName.value
                          )[0],
                      }
                    : {}
            )

            const { assetCount, isLoading: isAssetCountLoading } =
                useLinkedAssets(classification)
