<template>
    ...
    >
        <a-dropdown
            v-if="getActiveMenu(editor)?.key !== 'uploadimage' && editor.state.selection.from !== editor.state.selection.to"
            :trigger="['click']"
        >
        ...
        <a-popover v-if="getActiveMenu(editor)?.key !== 'uploadimage' && editor.state.selection.from !== editor.state.selection.to">
            <template #content>
...
<script lang="ts">
    ...
    import { blockMenu, menuDataTable, menuData } from '~/constant/readmeMenuItems'
