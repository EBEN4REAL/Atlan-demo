    ...
    setup(props, { emit }) {
        ...
        const saveQuery = async (saveQueryData: {
            ...
            const { data } = saveQueryToDatabaseAndOpenInNewTab(
                ...
                router,
                savedQueryType.value,
                saveQueryData.parentQF ?? getRelevantTreeData().parentQualifiedName.value,
                saveQueryData.parentGuid ?? getRelevantTreeData().parentGuid.value
            )
        ...
        const refetchParentNode = (
            ...
            if (!tree || tree === 'personal') per_refetchNode(per_guid, type)
