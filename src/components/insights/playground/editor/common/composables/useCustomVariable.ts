...
export function useCustomVariable(editorInstance?: any, monacoInstance?: any) {
    const { modifyActiveInlineTabEditor,modifyActiveInlineTab } = useInlineTab()
    ...
    function saveVariable(
        ...
        let index = activeInlineTab.value.playground.editor.variables.findIndex(variable=> variable.key ===currVariable.value.key)
        let savedIndex = activeInlineTab.value.playground.editor.savedVariables.findIndex(variable=> variable.key ===currVariable.value.key)
    ...
    function editVariable(
        ...
        let index = activeInlineTab.value.playground.editor.variables.findIndex(variable=> variable.name ===curr_variable.name)
        let savedIndex = activeInlineTab.value.playground.editor.savedVariables.findIndex(variable=> variable.name ===curr_variable.name)
    ...
    function addVariable(
        ...
        let index =null;
        index = activeInlineTab.value.playground.editor.savedVariables.findIndex(variable=> variable.name === new_variable.name)
        console.log('index: ', index)
        if(index!==-1) {
            let currIndex =  activeInlineTab.value.playground.editor.variables.findIndex(variable=> variable.name === new_variable.name)
            if(currIndex!==-1) {
                ...
                activeInlineTabCopy.playground.editor.variables = [...activeInlineTabCopy.playground.editor.variables, activeInlineTab.value.playground.editor.savedVariables[index]]
                editorInstance.trigger('keyboard', 'type', {
                    text: `{{${activeInlineTab.value.playground.editor.savedVariables[index].name}}}`,
                })
            }
        } else {
            activeInlineTabCopy.playground.editor.variables = [...activeInlineTabCopy.playground.editor.variables, new_variable]
            activeInlineTabCopy.playground.editor.savedVariables = [...activeInlineTabCopy.playground.editor.savedVariables, new_variable]
            editorInstance.trigger('keyboard', 'type', {
                text: `{{variable${len}}}`,
            })
        }     
    ...
    function deleteVariable(
        ...
        activeInlineTabCopy.playground.editor.variables = activeInlineTab.value.playground.editor.variables.filter(variable=> variable.key!==currVariable.key)
    ...
    function addVariableFromEditor(
        ...
        newVariables?.forEach(variable=> {
            index = activeInlineTab.value.playground.editor.savedVariables.findIndex(variable1=> variable1.name === variable.token.slice(2,-2))

            if(index!==-1) {
                data.push(activeInlineTab.value.playground.editor.savedVariables[index])
            } else {
        ...
        activeInlineTabCopy.playground.editor.variables = [...intersection, ...data]
        activeInlineTabCopy.playground.editor.savedVariables = [...activeInlineTabCopy.playground.editor.savedVariables, ...data]
