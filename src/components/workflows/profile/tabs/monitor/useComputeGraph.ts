...
export default function useComputeGraph(
    ...
    Object.values(n).forEach((v) => {
        if (v.type !== 'TaskGroup') {
            const { id, phase, displayName, startedAt, finishedAt, type } = v
            ...
            const nodeData = {
                ...
                html: {
                    render(node) {
                        ...
                        return `
                        <div class="monitor-node flex items-center ${data?.isSelectedNode === data?.id
                                ? 'isSelectedNode'
                                : ''
                            } ${phase}">
                            <div>
                                <svg width="30" height="30" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg" class="mr-3 animate-spin ${phase === 'Running' ? 'block' : 'hidden'
                            }">
                                ...
                                <svg width="22" height="22" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg" class="mr-3 ${phase === 'Succeeded' ? 'block' : 'hidden'
                            }">
                                ...
                                <svg width="22" height="22" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg" class="mr-3 ${['Failed', 'Error'].includes(phase)
                                ? 'block'
                                : 'hidden'
                            }">
                                ...
                                <svg width="26" height="26" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg" class="mr-3 ${['Omitted', 'Skipped'].includes(phase)
                                ? 'block'
                                : 'hidden'
                            }">
                                ...
                                <svg width="26" height="26" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg" class="mr-3 ${phase === 'Pending' ? 'block' : 'hidden'
                            }">
