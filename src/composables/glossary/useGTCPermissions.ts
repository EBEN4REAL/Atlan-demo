...
const useGTCPermissions = (GTC) => {
    ...
    const termAddPermission = computed(() => {
        ...
        const relationshipAddPermission = selectedAssetUpdatePermission(
            GTC.value,
            false,
            'RELATIONSHIP_ADD',
            'AtlasGlossaryTerm'
        )
        const entityCreatePermission = selectedAssetUpdatePermission(
            GTC.value,
            false,
            'ENTITY_CREATE'
        )
        return relationshipAddPermission && entityCreatePermission
    })
    ...
    const entityUpdatePermission = computed(() => {
        if (!GTC.value) return false
        return selectedAssetUpdatePermission(GTC.value, false, 'ENTITY_UPDATE')
    })

    const entityDeletePermission = computed(() => {
        if (!GTC.value) return false
        return selectedAssetUpdatePermission(GTC.value, false, 'ENTITY_DELETE')
    })

    const createPermission = computed(
        () => termAddPermission.value && categoryAddPermission.value
    )
...
export const fetchGlossaryPermission = (glossary, immediate = false) => {
    ...
    if (!glossary.value) return { isEvaluating: false, fetch: () => {} }
    const body = ref({})
    const authStore = useAuthStore()
    const { getAssetEvaluationsBody } = useAssetEvaluate()

    const { refresh, isLoading: isEvaluating } = useEvaluate(body, false, false)

    const fetch = () => {
        body.value = {
            entities: getAssetEvaluationsBody(glossary.value),
        }
        const permissionsAlreadyFetched = authStore?.evaluations?.some(
            (ev) => ev?.entityGuid === glossary.value?.guid
        )
        console.log(permissionsAlreadyFetched)
        console.log(useGTCPermissions(glossary).createPermission.value)
