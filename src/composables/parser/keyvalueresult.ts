...
import { PostgreSQLParserVisitor } from "~/grammar/PostgreSQLParserVisitor";
import { RootContext, KeyContext, ParserRuleContext, Values_stmtContext } from "~/grammar/PostgreSQLParserParser";
...
export class KeyValueResultVisitor extends AbstractParseTreeVisitor<KeyValueResult> implements PostgreSQLParserVisitor<KeyValueResult> {
    ...
    visitKeyValueExpression(node: KeyValueExpressionContext): KeyValueResult {
        ...
        const value = node.children?.find((child) => child instanceof Values_stmtContext);
