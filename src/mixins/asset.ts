    ...
    methods: {
        ...
        dataTypeImage(item: any) {
            const found = dataTypeList.find((d) =>
                d.type.includes(this.dataType(item))
            )
            return found?.image
        },
        ...
        assetTypeLabel(item: any) {
            const found = AssetTypeList.find(
                (d) => d.id === this.assetType(item)
            )
            return found?.label
        },
        ...
        integrationName(item: any): string {
            const name = this.attributes(item)?.integrationName
            ...
            if (found) {
                ...
                filtered.forEach((f) => {
                    temp.push({
                        ...f,
                        qualifiedName:
                            this.attributes(item)[f.qualifiedNameAttribute],
