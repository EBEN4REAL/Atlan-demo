...
export const ATLAN_BOT_CHECK = 'ATLAN_BOT_CHECK'
...
export const map = {
    ...
    [LIST_INTEGRATIONS_CONFIG]: () =>
        getAPIPath('/service', '/integrationtypes/configs'),
    ...
    [ATLAN_BOT_CHECK]: () => getAPIPath('/service', `/slack/atlanbot`),
    [JIRA_LIST_PROJECTS]: () => getAPIPath('/service', `/jira/projects`),
    [JIRA_LIST_ISSUE_TYPES]: () => getAPIPath('/service', `/jira/issueType`),
    [JIRA_GET_ISSUE_PROPERTY]: ({ issue, property }: PathParams) =>
        getAPIPath('/service', `/jira/issue/${issue}/properties/${property}`),
    [JIRA_GET_ISSUE_CONFIGURATIONS]: () =>
        getAPIPath('/service', `/jira/issue/configurations`),
    [JIRA_SEARCH]: () => getAPIPath('/service', `/jira/search`),
    [JIRA_LINK_ISSUE]: ({ id }: PathParams) =>
        getAPIPath('/service', `/jira/issue/link/${id}`),
    [JIRA_UNLINK_ISSUE]: ({ id }: PathParams) =>
        getAPIPath('/service', `/jira/issue/unlink/${id}`),
