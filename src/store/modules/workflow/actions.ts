...
export interface Actions {
  [ActionTypes.CONNECTOR_FETCH_LIST]({ commit }: AugmentedActionContext): void;
}

export const actions: ActionTree<State, RootState> & Actions = {
  async [ActionTypes.CONNECTOR_FETCH_LIST]({ commit, state }) {
    try {
      commit(MutationTypes.CONNECTOR_SET_STATUS, {
        loading: true,
      });
      ...
      if (response) {
        ...
        commit(MutationTypes.CONNECTOR_SET_LIST, data);
      }
    } catch (err) {
      if (err) {
        commit(MutationTypes.CONNECTOR_SET_STATUS, {
