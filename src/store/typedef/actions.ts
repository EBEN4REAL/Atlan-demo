...
export const actions: Actions = {
    ...
    updateSingleClassification(value: ClassificationInterface) {
        this.classificationList = this.classificationList.map(
            (classification) => {
    ...
    removeCustomMetadataByName(name: string) {
        this.customMetadataList = this.customMetadataList.filter(
            (cm) => cm.name !== name
        )
    },
    // enum list
    ...
    updateEnum(_enum) {
        const index =
            this.enumList.findIndex((e) => e.guid === _enum.guid) ?? -1
    ...
    updateCustomMetadata(bm) {
        const indexOfBmToUpdate = this.customMetadataList?.findIndex(
            (x) => x.guid === bm.guid
        )
        if (indexOfBmToUpdate > -1)
            this.customMetadataList[indexOfBmToUpdate] = bm
    },
    // states updates
    setIsLoading(value) {
