<template>
    <div class="relative flex flex-col justify-center w-full text-center">
        <AtlanLoader v-if="isLoading" class="absolute top-0 right-0 h-5" />
        <template v-if="mainStatText">
            <p class="text-5xl font-bold text-new-gray-800">
                {{ mainAggregation?.toLocaleString?.() }}
            </p>
            <p class="mb-6 text-sm text-gray-500">{{ mainStatText }}</p>
        </template>
